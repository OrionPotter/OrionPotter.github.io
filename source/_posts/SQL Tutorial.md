...
>索引是DBMS中辅助快速查找数据的辅助数据结构，类似于书籍的目录，快速定位所需信息
...
2. **维护开销**：插入、更新和删除操作需要同时更新索引，增加了维护开销。
3. **不适用于所有查询**：某些查询类型（如模糊查询、复杂的计算列查询）可能无法有效利用索引。
4. **索引选择性**：低选择性的列（如布尔值、性别等）创建索引效果不佳，可能会导致索引扫描而不是索引查找。
...
- **连接器**：
  ...
  - 连接数：
    ...
    - **空闲连接**：空闲连接数在`wait_timeout`以后断开连接。
    - **当前连接数**：`show processlist`
  - 长连接与短连接：
    - 长连接：
      - 优点：减少建立连接和断开连接的过程。
      - 缺点：占用内存增多。
      - 解决方案：定期断开长连接，客户端主动重置连接。
    - 短连接：
      - 缺点：多次建立连接浪费资源。

- **查询缓存**：

  - 查询缓存（Query cache）是以key-value形式存放在内存中，key为SQL语句，value为查询结果。
  - 缺点：对于更新频繁的表，查询缓存命中率低，MySQL 8已删除此功能。

- **解析器**：

  - 对SQL进行词法分析、语法分析，构建语法树，判断语法和词法是否正确。

- **执行SQL**：

  - **预处理器**：判断表和字段是否存在，扩展`select *`为所有列。

  - **优化器**：根据索引选择最佳查询执行计划。

  - 执行器

    ：根据执行计划执行SQL，从存储引擎读取记录，返回给客户端。

    - 执行过程：
      - 全表扫描：没有索引的情况下进行。
      - 主键索引查询：主键为索引的情况下进行。
      - 索引下推：存储引擎代替Server层进行判断过滤，减少二级索引查询时的回表操作，提高查询效率。
...
- **一致性（Consistency）**：通过原子性、隔离性和持久性保证。
- **隔离性（Isolation）**：通过MVCC（多版本并发控制）或锁机制保证。
- **持久性（Durability）**：通过`redo log`（重做日志）保证。

### 并发事务引发问题

- **脏读**：读到其他事务未提交的数据。
- **不可重复读**：一个事务多次读取同一个数据，前后两次读到的值不一致。
- **幻读**：一个事务同一个筛选条件进行的查询记录，前后两次得到的记录数量不一致。

### 事务的隔离级别

- **读未提交**：一个事务未提交时，它做的变更其他事务就能看到。
- **读已提交**：一个事务提交后，它做的变更其他事务才能看到。
- **可重复读**：一个事务开启后，每次读取到的数据都和开启事务之后的数据保持一致。
- **序列化**：会对记录进行加锁，当其他事务进行写操作时，由于未拿到锁，会处于阻塞状态，直到释放锁。

### Read View在MVCC如何工作

- **Read View**：在事务开始时创建，记录当前活跃事务的ID，用于判断数据的可见性。

### 可重复读是如何工作的

- **可重复读**：在事务开始时创建Read View，每次读取数据时，使用Read View判断数据的可见性，确保每次读取的数据一致。

### 读提交是如何工作的

- **读提交**：每次读取数据时创建新的Read View，确保读取到的数据是最新的已提交数据。

## 索引

### 什么是索引？

- **索引**：帮助存储引擎快速获取数据的一种数据结构。

### 索引的分类

- **数据结构**：
  - **B+树索引**：常见的索引结构，适用于范围查询。
  - **Hash索引**：基于哈希表的索引结构，适用于等值查询。
  - **Full-text索引**：全文索引，适用于全文搜索。
- **物理存储**：
  - **聚簇索引（主键索引）**：数据按主键顺序存储，主键索引即数据本身。
  - **二级索引（辅助索引）**：非主键索引，索引记录存储主键值。
- **字段特性**：
  - **主键索引**：唯一标识数据行，不能有重复值。
  - **唯一索引**：保证字段值唯一，可以有一个NULL值。
  - **普通索引**：没有唯一性约束。
  - **前缀索引**：对字符串的前缀部分创建索引，节省空间。
- **字段个数**：
  - **单列索引**：单个字段的索引。
  - **联合索引**：多个字段的组合索引。
...
- **使用覆盖索引**：查询只访问索引，不访问数据行。
- **合理选择索引列顺序**：将选择性高的列放在前面。
- **避免冗余和重复索引**：减少不必要的索引。
- **使用前缀索引**：对长字符串字段使用前缀索引，节省空间。
...
- **全局锁**：
  - **全库逻辑备份**：对整个数据库加锁，防止其他操作。
- **表级锁**：
  - 表锁：
    - 共享锁（读锁）：多个事务可以同时读取表。
    - 独占锁（写锁）：一个事务独占表，其他事务不能访问。
  - 元数据锁（MDL）：
    - 对表进行CRUD操作时，加MDL读锁。
    - 对表进行结构变更操作时，加MDL写锁。
    - MDL在事务提交后才会释放，这意味着事务执行期间，MDL是一直持有的。由于写锁获取优先级高于读锁，写锁获取不到会阻塞整个表的读操作。
- **意向锁**：
  - 意向锁是一种表级锁，用于指示事务即将对表中的某些行加锁。
  - 意向共享锁（IS）：事务准备对表中的某些行加共享锁。
  - 意向独占锁（IX）：事务准备对表中的某些行加独占锁。
- **AUTO-INC锁**：
  - 自增锁（AUTO-INC锁）用于保护自增列的生成。
  - 在插入新记录时，确保每个事务生成唯一的自增值。
- **行级锁**：
  - **记录锁（Record Lock）**：锁定单个记录。
  - **间隙锁（Gap Lock）**：锁定一个范围，不包括记录本身，用于防止幻读。
  - **Next-Key Lock**：记录锁和间隙锁的组合，锁定一个范围，包括记录本身。
